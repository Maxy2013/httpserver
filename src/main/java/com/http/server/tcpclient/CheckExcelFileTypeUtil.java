...
//		System.out.println(mFileTypes.get(getFileHeader(filePath)));
    ...
    private static String bytesToHexString(byte[] src) {
        ...
        for(byte by: src){
            ...
            hv = Integer.toHexString(by & 0xFF).toUpperCase();
            if (hv.length() < 2) {
                builder.append(0);
            }
            builder.append(hv);
...
//        final String fileType = getFileType("D:/BugReport - 副本.xlsx");
//
//
//        String fileHeader = getFileHeader("D:/BugReport - 副本.xlsx");
//        String fileHeader2 = getFileHeader("D:/BugReport - 副本.txt");
//        System.out.println(fileType);
//        System.out.println(fileHeader);
//        System.out.println(fileHeader2);

        String filePath = "D:/售电量aaa - 副本.xlsx";
        String suffix = filePath.substring(filePath.lastIndexOf(".") + 1, filePath.length());
        System.out.println("后缀名是：" + suffix);


        //上传文件格式和记录数校验
        String fileType1 = CheckExcelFileTypeUtil.getFileType(filePath);
        if (!suffix.equals("xlsx") || !suffix.equals(fileType1)) {
